...
import { Globe2, DollarSign, TrendingUp, RefreshCcw } from "lucide-react";
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
      <Card className="bg-blue-600 dark:bg-blue-900 border-blue-500 dark:border-blue-800 text-white shadow-sm overflow-hidden relative group">
        <div className="absolute -right-6 -top-6 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
          <Globe2 className="h-40 w-40 text-blue-200 dark:text-blue-800" />
        </div>
        ...
        <CardContent className="p-6 flex flex-col justify-between h-full relative z-10">
          <div>
            <p className="text-blue-100 text-xs font-bold uppercase tracking-wider mb-1 opacity-80 flex items-center gap-1">
              <Globe2 className="h-3 w-3" /> Sistema internacional
            </p>
            <h3 className="text-2xl font-bold text-white mb-1">Montos flexibles por expediente</h3>
            <p className="text-blue-100/70 text-xs flex items-center gap-1">
              <RefreshCcw className="h-3 w-3" /> El estudio define importes y referencias monetarias sin unidades locales fijas
            </p>
          </div>
          <div className="mt-4">
            <p className="text-lg font-semibold text-white/90 leading-relaxed">
              Registra honorarios, presupuestos y estimaciones segun pais, moneda y criterio profesional.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-emerald-600 dark:bg-emerald-900 border-emerald-500 dark:border-emerald-800 text-white shadow-sm relative overflow-hidden group">
        ...
        <CardContent className="p-6 flex flex-col justify-between h-full relative z-10">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-emerald-100 text-xs font-bold uppercase tracking-wider mb-1 opacity-80 flex items-center gap-1">
                <TrendingUp className="h-3 w-3" /> Cotizacion en vivo
              </p>
              <h3 className="text-2xl font-bold text-white">Dolar blue</h3>
            </div>
          ...
          <p className="text-[10px] text-emerald-100/60 mt-2 text-right">Fuente: DolarApi.com</p>
        </CardContent>
      </Card>
    </div>
  );
