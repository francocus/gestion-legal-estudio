...
function EventRow({ event, onToggle, showDelete = true }: {
  ...
  return (
    <div className={`group flex items-center gap-4 rounded-xl border p-4 shadow-sm transition-colors ${
      isOverdue
      ...
      <button type="button" onClick={() => onToggle(event.id, event.isDone)} className="text-gray-300 hover:text-green-500 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-1 flex flex-wrap gap-2 text-xs text-gray-500">
          ...
          {event.case?.client && (
            <>
              <span>-</span>
              <span>{event.case.client.lastName}, {event.case.client.firstName}</span>
            </>
          )}
        </div>
      </div>
...
export function AgendaPanel({ initialEvents, activeCases }: { initialEvents: AgendaEvent[]; activeCases: AgendaCaseOption[] }) {
  ...
  const [filter, setFilter] = useState<"ALL" | "URGENT" | "CASE" | "PERSONAL" | "DONE">("ALL");
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          ...
          <div className="mt-4 rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-blue-50 p-4 dark:border-indigo-900/40 dark:from-indigo-950/30 dark:to-blue-950/20">
            ...
            <div className="mt-2 text-sm leading-6 text-slate-700 dark:text-slate-300">
              {todayRangeCount > 0
                ? `Tenes ${todayRangeCount} compromiso${todayRangeCount > 1 ? "s" : ""} para hoy y ${pendingUrgentCount} evento${pendingUrgentCount > 1 ? "s" : ""} urgente${pendingUrgentCount > 1 ? "s" : ""} dentro del corto plazo.`
                : `No hay compromisos para hoy. Quedan ${pendingUrgentCount} evento${pendingUrgentCount > 1 ? "s" : ""} urgente${pendingUrgentCount > 1 ? "s" : ""} en la agenda proxima.`}
            </div>
          </div>
        </div>
      </div>
